    ...
    for (let i = 0; i < bothInningArr.length; i++) {
        ...
        for(let j=0;j<batsmanArr.length;j++) {
            ...
            if(cols.length !== 8) continue ;
             let playerName = searchTool(cols[0]).text();
            ...
            processPlayer(teamName,playerName,runs,balls,fours,sixes,sr,opponentName,date,venue);
        }
        console.log("***********************************")
    }
}

function processPlayer(teamName,playerName,runs,balls,fours,sixes,sr,opponentName,date,venue) {
    ...
    let playerObj={
     ...
     if(fs.existsSync(filePath)==false) {
        content.push(playerObj)
         ...
         content = excelReader(filePath, playerName);
        content.push(playerObj);
    }
